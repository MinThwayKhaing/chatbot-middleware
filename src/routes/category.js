...
router.get('/', async (req, res) => {
  try {
    ...
    const response = await listKnowledgeBasesWithPagination(pageSize, pageToken);
...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Knowledge Base created', data: kb });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Knowledge Base updated', data: updatedKb });
  } catch (error) {
